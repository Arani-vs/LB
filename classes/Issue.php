...
class Issue {
    ...
    public function issueBook() {
        ...
        $sql = "INSERT INTO " . $this->table_name . " (book_no, book_name, book_author, student_id, status, expected_return_date) VALUES (?, ?, ?, ?, ?, ?)";
        $stmt = $this->conn->prepare($sql);
        $stmt->bind_param("issiis", $this->book_no, $this->book_name, $this->book_author, $this->student_id, $status, $this->expected_return_date);
        if($stmt->execute()){
            return true;
        }
        return false;
    ...
    public function getIssuedBooksByUser($student_id) {
    ...
    public function getAllIssuedBooks() {
        $sql = "SELECT i.*, u.name as student_name FROM " . $this->table_name . " i
                LEFT JOIN " . $this->table_users . " u ON i.student_id = u.id";
